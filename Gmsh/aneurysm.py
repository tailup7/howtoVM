...
if extrude:
    ...
    e1 = gmsh.model.geo.extrudeBoundaryLayer(gmsh.model.getEntities(2), [4], [0.5], True)

    # extrude a second boundary layer in the opposite direction (note the `second == True' argument to distinguish it from the first one)
    e2 = gmsh.model.geo.extrudeBoundaryLayer(gmsh.model.getEntities(2), [4], [-0.5], True, True)
